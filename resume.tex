  ...
  \resumeSubHeadingListStart
    \resumeSubheading
      ...
      {B.S. Computer Engineering (Magna Cum Laude), M.S. Intelligent Systems, Robotics \& Control}{June 2024}
        \resumeItemListStart
            \resumeItem{Relevant Coursework}{Robot System Design \& Implementation (kinematics, dynamics, control), Planning \& Learning, Sensing \& Estimation, Autonomous Vehicles, Computer Vision, Deep Learning, Software Engineering, Operating Systems, Statistical Learning}
        \resumeItemListEnd
...
\section{Experience}
  ...
  \resumeSubheading
      ...
      \resumeItemListStart
        ...
        \resumeItem{Multi-Axis Robot Integration}
          {Integrated and programmed \textbf{6-DOF industrial articulated robot arms} for additive manufacturing, implementing \textbf{forward/inverse kinematics}, tool/base frame transformations, and \textbf{trajectory generation} for coordinated motion planning}
        \resumeItem{Robot-Sensor Coordination}
          {Integrated Gocator 2D laser profilometer with 6-DOF robot motion planning; implemented real-time \textbf{RANSAC circle fitting} at \textbf{10Hz} and \textbf{ICP registration} to compensate for robot positioning inaccuracies, increasing data completeness \textbf{20x} and reducing noise by \textbf{50\%}}
        \resumeItem{End-Effector Control}
          {Developed real-time control software for welding torch end-effectors synchronized with 6-axis robot motion, achieving \textbf{1ms control loop latency} for closed-loop sensor-robot coordination}
        \resumeItem{Closed-Loop Control}
          {Designed and implemented \textbf{PID controllers with anti-windup} for wire feed speed, reducing bead variation by \textbf{30\%}; integrated time-based adaptive parameter control for real-time weld process optimization}
        ...
        \resumeItem{Experimental Design}
          {Designed and executed \textbf{6+ weld trials}; logged all telemetry to cloud for dataset generation, performed \textbf{Python} data analysis to support simulation team and future learning-based optimization}
      \resumeItemListEnd
  \resumeSubheading
      {Relativity Space}{Long Beach, CA}
    ...
    \resumeSubheading
      ...
      \resumeItemListStart
        ...
        \resumeItem{Data Pipeline}
          {Designed a robust async data pipeline with the \textbf{ZeroMQ} Python API for the wireless display, transmission, storage (\textbf{PostgreSQL}), analysis of neural data for an array of \textbf{100+ medical devices (\texttt{$\sim$}155 Mbps)}, creating infrastructure suitable for ML training workflows}
        \resumeItem{Project Management}
...
\section{Skills}
 \resumeSubHeadingListStart
   ...
   \vspace{-2pt}\item
        \textbf{Technologies:}{ AWS, Git, Jupyter Notebook, Linux, Docker, MATLAB/Simulink, Confluence, Jira, Gerrit, Gitlab CI, Claude }
    \vspace{-5pt}

    \vspace{-2pt}\item
        \textbf{Frameworks:}{ ROS2, OPC-UA, PyTorch, MQTT, Zenoh, Xenomai, EtherCAT, NumPy, Pandas, SciKit, Ansible }
    \vspace{-5pt}
 \resumeSubHeadingListEnd
